...
from data_utils import ShoesDataset, base_path, squarepad_transform, targetpad_transform, FashionIQDataset,CIRRDataset
from utils import collate_fn, update_train_running_results, extract_index_features, \
...
def clip_finetune_fiq(train_dress_types: List[str], val_dress_types: List[str],
    ...
    training_path: Path = Path(
        base_path / f"models-finetune/fiq_{clip_model_name}_{training_start}")
    ...
    if args.clip_model_ckpt!='':
        print('Trying to load the CLIP model ckpt')
        clip_model_path = args.clip_model_ckpt
        saved_state_dict = torch.load(clip_model_path, map_location=device)
        clip_model.load_state_dict(saved_state_dict["CLIP"])
        print('CLIP model ckpt loaded successfully')
    else:
        print('Do not load clip_model_ckpt')
    ...
    for epoch in range(num_epochs):
        ...
        for idx, (reference_images, target_images, captions,_) in enumerate(relative_train_loader):
...
def clip_finetune_shoes(num_epochs: int, clip_model_name: str, learning_rate: float, batch_size: int,
                      validation_frequency: int, transform: str, save_training: bool, encoder: str, save_best: bool,
                      **kwargs):
    training_start = datetime.now().strftime("%Y-%m-%d_%H:%M:%S")
    training_path: Path = Path(
        base_path / f"models-finetune/clip_shoes_{clip_model_name}_{training_start}")
    training_path.mkdir(exist_ok=False, parents=True)
    ...
    for epoch in range(num_epochs):
        ...
        if epoch%validation_frequency==0:
            if encoder!='text':
                val_index_features,val_index_names=extract_index_features(classic_val_dataset,clip_model)
            recall_at1,recall_at10,recall_at50=compute_shoes_val_metrics(relative_val_dataset,clip_model,val_index_features,val_index_names,combining_function)
            results_dict={
...
def clip_finetune_cirr(num_epochs: int, clip_model_name: str, learning_rate: float, batch_size: int,
    ...
    training_start = datetime.now().strftime("%Y-%m-%d_%H:%M:%S")
    training_path: Path = Path(
        base_path / f"models-finetune/cirr_{clip_model_name}_{training_start}")
    training_path.mkdir(exist_ok=False, parents=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset", type=str, required=True, help="should be either or 'fashionIQ' or 'shoes'")
    ...
    parser.add_argument('--clip_model_ckpt',default='')
    ...
    if args.dataset.lower() == 'fashioniq':
        ...
        clip_finetune_shoes(**training_hyper_params)
