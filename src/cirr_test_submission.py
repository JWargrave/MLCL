...
def generate_cirr_test_submissions(combining_function: callable, file_name: str, clip_model: CLIP,
    ...
    submissions_folder_path = base_path / "submission" / 'CIRR' / file_name
...
def main():
    ...
    parser.add_argument('--learn_t',default=0,type=int)
    ...
    if args.combining_function.lower() == 'sum':
        ...
        combiner = Combiner(feature_dim, args.projection_dim, args.hidden_dim,args.learn_t).to(device)
        saved_state_dict = torch.load(args.combiner_path, map_location=device)
        combiner.load_state_dict(saved_state_dict["Combiner"])
        combiner.eval()
        combining_function = combiner.combine_features_weighted
    else:
