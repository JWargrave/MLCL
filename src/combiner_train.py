...
from data_utils import ShoesDataset, base_path, squarepad_transform, FashionIQDataset, targetpad_transform,CIRRDataset
from combiner import Combiner
from utils import collate_fn, update_train_running_results, save_model, \
    extract_index_features, generate_randomized_fiq_caption, device
from validate import compute_fiq_val_metrics,compute_shoes_val_metrics,compute_cirr_val_metrics
...
def combiner_training_fiq(train_dress_types: List[str], val_dress_types: List[str],
    ...
    training_path: Path = Path(
        base_path / f"dev-models-combiner/fiq_learnT_{args.learn_t}_{clip_model_name}_{training_start}")
    ...
    combiner = Combiner(feature_dim, projection_dim, hidden_dim,args.learn_t).to(device, non_blocking=True)

    # ppp=''
    # if ppp!='':
    #     print('Load Combiner.....',ppp)
    #     combiner.load_state_dict(torch.load(ppp,map_location=device)['Combiner'])
    # else:
    #     print('Do not load Combiner....')
    ...
    optimizer=optim.Adam([
        {
            'params':[p for _,p in filter(lambda p:p[0]!='loss_weight',combiner.named_parameters())],
            'lr':combiner_lr
        },
        {
            'params':[p for _,p in filter(lambda p:p[0]=='loss_weight',combiner.named_parameters())],
    ...
    for epoch in range(num_epochs):
        ...
        if epoch%1==0:
            print('2*torch.sigmoid(combiner.loss_weight):',2*torch.sigmoid(combiner.loss_weight))
        ...
        combiner.train()
        for idx, (reference_images, target_images, captions,reference_images_aug) in enumerate(relative_train_loader):  # Load a batch of triplets
            images_in_batch = reference_images.size(0)

            optimizer.zero_grad()

            reference_images = reference_images.to(device, non_blocking=True)
            reference_images_aug=reference_images_aug.to(device, non_blocking=True)
            ...
            with torch.no_grad():
                ...
                reference_images_aug_list=torch.split(reference_images_aug, clip_bs)
                reference_features_aug=torch.vstack(
                    [clip_model.encode_image(mini_batch).float() for mini_batch in reference_images_aug_list])
            ...
            with torch.cuda.amp.autocast():
                logits1,logits2,logits3,mid_raw_cross_modal_logits,cross_modal_logits_1 = combiner(reference_image_features, text_features, target_image_features,reference_features_aug)
                ground_truth = torch.arange(images_in_batch, dtype=torch.long, device=device)
                loss = 2*torch.sigmoid(combiner.loss_weight)*crossentropy_criterion(logits1, ground_truth)+\
                        (2-2*torch.sigmoid(combiner.loss_weight))*crossentropy_criterion(logits2, ground_truth)+\
                            crossentropy_criterion(logits3, ground_truth)+\
                        crossentropy_criterion(mid_raw_cross_modal_logits,ground_truth)+\
                        crossentropy_criterion(cross_modal_logits_1,ground_truth)
        ...
        if epoch % validation_frequency == 0:
            ...
            for relative_val_dataset, index_features, index_names, idx in zip(relative_val_datasets,
                ...
                recall_at10, recall_at50 = compute_fiq_val_metrics(relative_val_dataset, clip_model, index_features,
                                                                    index_names, combiner.combine_features_weighted)
...
def combiner_training_shoes(projection_dim: int, hidden_dim: int, num_epochs: int, clip_model_name: str,
    ...
    training_start = datetime.now().strftime("%Y-%m-%d_%H:%M:%S")
    training_path: Path = Path(
        base_path / f"dev-models-combiner/shoes_learnT_{args.learn_t}_{batch_size}_{clip_model_name}_{training_start}")
    training_path.mkdir(exist_ok=False, parents=True)
    print('training_path:',training_path)
    with open(training_path / "training_hyperparameters.json", 'w+') as file:
        json.dump(training_hyper_params, file, sort_keys=True, indent=4)
    ...
    scheduler=CosineAnnealingWarmRestarts(optimizer=optimizer,T_0=10,T_mult=1,eta_min=1e-6,last_epoch=-1) if args.use_schedule==1 else None
    ...
    for epoch in range(num_epochs):
        ...
        train_running_results = {'images_in_epoch': 0, 'accumulated_train_loss': 0}
        combiner.train()
        iters_num=len(relative_train_loader)
        for idx, (reference_images, target_images, captions,reference_images_aug) in enumerate(relative_train_loader):  # Load a batch of triplets
            images_in_batch = reference_images.size(0)
            ...
            with torch.no_grad():
                ...
                text_features = torch.vstack(
                    [clip_model.encode_text(mini_batch).float() for mini_batch in text_inputs_list])
            with torch.cuda.amp.autocast():
                logits1,logits2,logits3,mid_raw_cross_modal_logits,cross_modal_logits_1 = combiner(reference_features, text_features, target_features,reference_features_aug)
                ground_truth = torch.arange(images_in_batch, dtype=torch.long, device=device)
                loss = 2*torch.sigmoid(combiner.loss_weight)*crossentropy_criterion(logits1, ground_truth)+\
                        (2-2*torch.sigmoid(combiner.loss_weight))*crossentropy_criterion(logits2, ground_truth)+\
                            crossentropy_criterion(logits3, ground_truth)+\
            ...
            if scheduler is not None:
                scheduler.step(epoch+idx/iters_num)
        ...
        if epoch%validation_frequency==0:
            ...
            clip_model = clip_model.float()  # In validation we use fp32 CLIP model
            combiner.eval()
            recall_at1,recall_at10,recall_at50=compute_shoes_val_metrics(relative_val_dataset,clip_model,val_index_features,val_index_names,combiner.combine_features_weighted)
            results_dict={
                'recall_at1':recall_at1,
                'recall_at10':recall_at10,
                'recall_at50':recall_at50,
                'recall_mean':(recall_at1+recall_at10+recall_at50)/3
            }
            print(json.dumps(results_dict, indent=4))
            log_dict = {'epoch': epoch}
            log_dict.update(results_dict)
            ...
            validation_log_frame.to_csv(str(training_path / 'validation_metrics.csv'), index=False)
            if save_training:
                if save_best and results_dict['recall_mean']>best_avg_recall:
                    best_avg_recall=results_dict['recall_mean']
...
def combiner_training_cirr(projection_dim: int, hidden_dim: int, num_epochs: int, clip_model_name: str,
    ...
    training_start = datetime.now().strftime("%Y-%m-%d_%H:%M:%S")
    training_path: Path = Path(
        base_path / f"dev-models-combiner/cirr_learnT_{args.learn_t}_{clip_model_name}_{training_start}")
    training_path.mkdir(exist_ok=False, parents=True)
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % validation_frequency == 0:
            ...
            results = compute_cirr_val_metrics(relative_val_dataset, clip_model, val_index_features,
                                                val_index_names, combiner.combine_features_weighted)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset", type=str, required=True, help="should be either 'fashionIQ' or 'Shoes'")
    ...
    parser.add_argument('--combiner_model_path',type=str)

    parser.add_argument('--use_schedule',default=0,type=int)
    parser.add_argument('--learn_t',default=1,type=int)


    args = parser.parse_args()
    if args.dataset.lower() not in ['fashioniq','shoes','cirr']:
        raise ValueError("Dataset should be either 'FashionIQ' or 'Shoes' or 'CIRR'")
